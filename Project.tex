...
\begin{figure}[H]
	\includegraphics[scale = 0.17]{Figures/nya4_20110106_0000-2400_5577.png}
	\includegraphics[scale = 0.17]{Figures/nya4_20110106_0000-2400_6300.png}
	\centering
	\caption{Keograms showing development of aurora over Ny Ålesund the sixth of february 2011. Source: \url{http://tid.uio.no/plasma/aurora/}}
	\label{fig:keogram}
\end{figure}
...
If we look at the data from SWEPAM and start with the Proton density, Figure \ref{fig:ACE_H_Density}, we see that it is stable at 10 protons per cubic centimeter until 16:00 hours, where it makes a jump up and keeps climbing and reaches a maximum at 70$/cc$ at right before 20:00 hours. After this it drops down to around 20$/cc$ for the rest of the day.
...
Beginning with some of the most important part of the data, the IMF, we see in the subsection \ref{sub:ACE_data} and from Figure \ref{fig:ACEmag} that the IMF Bz becomes negative at 20UT. This is great, because we know that when the IMF Bz is negative then reconnection can happen, which gives us a lot of fun physical processes. We notice that all the other measurements have an equivalent reaction. Such as the solar wind velocity has big changes with the bulk speed increasing from 350 km/s to 450km/s. 
\\
The radial proton temperature increases quite a bit just as the proton density drops down dramatically. 
\\
All this suggests that there has been some sort of ejection from the sun some time before, which is just now hitting ACE.
\\
Based on this we expect to see it hitting the Earth after an hour due to the velocity and the distance. Since the IMF Bz is negative we expect reconnection which has several effects on the Earths magnetic field and we can study these effects with indices such as Dst,AE and Kp.\\
...
\\
This can also be seen in the AE index. around 21:30 UT the substorm starts and begins to grow. It reaches the expansion phase and a maximum just before midnight. This means that there is a strong electojet westwards. The magnitude of this geomagnetic storm is represented by the Kp index which hase a value of 4+, which makes it a moderate storm.


\textbf{IMAGE}\\
As we have a solar ejection hitting the earth with a negative IMF $B_Z$, we expect to see an influx of ionospheric currents after some time. One way this would be measured would be using IMAGE. If the magnetic reconnection caused a dawn-dusk current, we would see a negative $B_X$. If we got a Hall-current, we would see a positive $B_Y$.\\
From Figure \ref{fig:image_data} we see that this is exactly what happens. Approximately 2 hours after ACE measured the solar ejection, about 1 hour after it would have hit Earth, both the X and Y components see a spike.\\
We also see a the Z component spiking, which, together with the X component data, supports what we are reading from DST and AE.

\textbf{SuperDARN}\\
Much like IMAGE, we would expect SuperDARN readings to increase during times of magnetic reconnection.\\
From \ref{fig:superdarn_data} we can see that the Hall-current increases massively at around 21:50 UT, which is at the same time as the IMAGE data kicks in. We can also see that the polar cusp expands as the substorm commences. 

\textbf{ASI}\\
As we are experiencing a solar ejection with a negative IMF $B_Z$, we can expect there to be auroras some time after the solar wind passes ACE. As there seemed to be little activity beforehand, we would also expect there to be little to no aurora before the ejection hits earth.\\
From figure \ref{fig:keogram} there would at first glance seem to be some activity in the time before the indicents occur. However, the intensity seems to be moving longitudinal in a way that indicates a cloudy sky, meaning that the data is bad. In other words, we cant really say anything about what the keogram is showing before things really escalate around 22-23 UT. At that point, the whole keogram seems to turn an intense red, indicating large auroral activity in both wavelengths. Again, this supports the other data which is pointing towards ionospheric activity after around 22 UT. \\
...
The data from ACE shows IMF Bz becoming negative and the solar wind velocity increasing, which point to some sort of solar ejection that has passed the satellite and is headed towards the earth. Upon arrival it causes a geomagnetic substorm which is observed on the Dst, AE and Kp indices. It also creates an influx of ionospheric currents which we can see with IMAGE and SuperDARN. Before it culminates with a display of Aurora Borealis.
...
Russel C.T, et.al(2016) Space Physics An Introduction.\\
