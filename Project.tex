...
\textbf{SuperDARN}\\
We see from the data that the OCB increases in size starting with the lowest lattitude being just below 70$^\circ$, which is roughly the top of Norway, and ending up at a lattitude of 50$^\circ$, reaching Germany. This is due to the reconnection of the IMF and the Earths magnetic field, which causes flux transfer into the OCB, and since it is not open on the other side, it becomes like a balloon in which the amount of air you blow into it is greater than the amount of air that escapes the hole resulting in a bigger balloon.
