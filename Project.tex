...
\label{sub:theory}
	\subsubsection{The Sun:} 
		...
		There are events where the sun sends out relatively huge amounts of plasma.\\
		One such event is a solar flare. A solar flare is caused by energy building up in the suns lower atmosphere, which is then released (likely connected to 				the magnetic field) causing a spike in the solar wind density. The size of solar flares vary, but they are relatively small compared to what we will 					discuss next; the CME.\\
		A coronal mass ejection (CME) is an event where an enourmous amount of charged particles is accelerated away from the corona. The size of a CME 				can be many times the size of the sun, making for some pretty looking pictures.
		\begin{figure}[H]
